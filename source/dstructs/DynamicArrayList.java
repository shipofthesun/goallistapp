package javaprojects.goallistapp.source.dstructs;
...
public class DynamicArrayList<T> {
	...
	**/
	DyanamicArray(int initialLength) {
		dArray = new T[initialLength];
		listSize = 0;
	}
	...
	**/
	...
	{
		for(int dArrayPos = listSize, int listPos = 0; dArrayPos < listSize + list.getSize(); dArrayPos++, listPos++)
			insert(array.getElement(listPos),dArrayPos);
	}
